...
function moveTopLeftOutgoingForward(node, grid, executionSequence, graph, visited) {
  ...
  while (existingEdges.length > 0) {
    ...
    for (const element of gridCopy.elements) {
      ...
      if (elementPos[1] < targetPos[1] || elementPos[0] > sourcePos[0]) gridCopy.removeElement(element);
    }
    ...
    //   if (index === cord[0]) grid.expandRow(cord[0], cord[1] - 1, shift);
    // })
    ...
    for (let rowIndex = 0; rowIndex < grid.rowCount; rowIndex++) {
      ...
      // if (rowIndex >= sourcePos[0]) {
      //   grid.expandRow(rowIndex, sourcePos[1], shift);
      //   continue;
      // }
      const getShiftPos = () => {
