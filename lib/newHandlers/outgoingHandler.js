...
function getInsertPosition(element, grid, nextEl) {
  ...
  const allItemsInElementPosition = [ ...grid.get(elPos[0], elPos[1]) ].filter(item => item.$type !== 'bpmn:Lane' && item.$type !== 'bpmn:Process');
  ...
  if (grid.isCrossed([ position[0], position[1] ])) {
    ...
    const sourceLane = nextEl.laneRef;
    const [ sourceLaneRow ] = grid.find(sourceLane) || [];
    grid.addRowCol(false, position[0] - 1);

    if (sourceLane && position[0] === sourceLaneRow) {
      const pos = grid.find(sourceLane);
      pos[3] += 1;
      grid.move(sourceLane, [ pos[0] - 1, pos[1] ]);
    }
...
function fixNewVerticalCrosses(edge, grid) {
  ...
  const vCrossed = grid.getCrossedElementsFor(edge, true).filter(item => item.$type !== 'bpmn:Lane' && item.$type !== 'bpmn:Process');
...
function fixNewHorizontalCrosses(edge, grid) {
  ...
  const hCrossed = grid.getCrossedElementsFor(edge, false).filter(item => item.$type !== 'bpmn:Lane' && item.$type !== 'bpmn:Process');
  ...
  if (direction === 'SW_NE') {
    ...
    const sourceLane = grid.getEdgeSource(edge).laneRef;
    const [ sourceLaneRow ] = grid.find(sourceLane) || [];
    grid.addRowCol(false, baseSourceRow - 1, maxDown);

    if (sourceLane && baseSourceRow === sourceLaneRow) {
      const pos = grid.find(sourceLane);
      pos[3] += maxDown;
      grid.move(sourceLane, [ pos[0] - maxDown, pos[1] ]);
    }
