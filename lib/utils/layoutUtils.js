...
export function getBounds(element, elementPosition, shift, attachedTo) {
  ...
  const width = ((!positionWidth ? 1 : positionWidth) - 1) * DEFAULT_CELL_WIDTH + defaultWidth;
  const height = ((!positionHeight ? 1 : positionHeight) - 1) * DEFAULT_CELL_HEIGHT + defaultHeight;

  const { x, y } = shift;
  return {
    width,
    height,
    x: col * DEFAULT_CELL_WIDTH + (DEFAULT_CELL_WIDTH - defaultWidth) / 2 + x,
    y: row * DEFAULT_CELL_HEIGHT + (DEFAULT_CELL_HEIGHT - defaultHeight) / 2 + y
  };
}
