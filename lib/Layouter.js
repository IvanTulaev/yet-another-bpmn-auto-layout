...
export class Layouter {
  ...
  bindOldDi() {
    for (const diagram of this.diagram?.diagrams || []) {
      for (const oldDi of diagram.plane?.planeElement || []) {
        if (oldDi.bpmnElement) {
          oldDi.bpmnElement.oldDi = oldDi;
        }
      }
    }
  }
  ...
  drawStraight(message) {
    ...
    const options = message.$type === 'bpmn:MessageFlow' ? { ...message.oldDi, id: `${message.id}_di` } : { ...message.originalEdge.oldDi };
    delete options.waypoint;
    delete options.label;
    delete options.$type;

    const edge = this.diFactory.createDiEdge(message, waypoints, options);
  ...
  createParticipantDi(participant, origin) {
    ...
    const attributes = { ...participant.oldDi, id: participant.id + '_di' };
    delete attributes.bounds;
    delete attributes.label;
    delete attributes.$type;
    const participantDi = this.diFactory.createDiShape(participant, { width, height, ...origin }, attributes);
    ...
    nestedLanes.nestedSet.forEach((pos, lane) => {
      ...
      const attributes = { ...lane.oldDi, id: lane.id + '_di' };
      delete attributes.bounds;
      delete attributes.label;
      delete attributes.$type;

      const laneDi = this.diFactory.createDiShape(lane, { width: laneWidth, height:laneHeight, x: laneX, y: laneY }, attributes);
      const planeDi = this.diagram.diagrams[0].plane.get('planeElement');
      planeDi.push(...[ laneDi ]);
    });
  ...
  createGridLayout(process) {
    ...
    for (const node of processGraph.nodes) {
      ...
      for (const outgoingItem of node.outgoing || []) {
        ...
        newEdge.originalEdge = outgoingItem;
      ...
      for (const association of dataInputAssociations || []) {
        for (const dataSource of association.sourceRef || []) {
          ...
          newEdge.originalEdge = association;
