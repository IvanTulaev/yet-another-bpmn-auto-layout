...
export class Layouter {
  ...
  drawProcesses() {
    ...
    // const sortedProcesses = procTree.nodes.sort((a, b) => b.level - a.level);
  ...
  createGridLayout(process) {
    ...
    for (const node of processGraph.nodes) {
      ...
      if (node.$type === 'bpmn:BoundaryEvent') {
        processGraph.addEdge({ source: node, target: node.attachedToRef });
        processGraph.addEdge({ source: node.attachedToRef, target: node });
      }
      ...
      for (const outgoingItem of node.outgoing || []) {
        processGraph.addEdge({ source:outgoingItem.sourceRef, target:outgoingItem.targetRef, id: outgoingItem.id });
      }
      ...
      for (const association of dataInputAssociations || []) {
        for (const dataSource of association.sourceRef || []) {
          processGraph.addEdge({ source:dataSource, target: association.$parent, id: association.id, propRef: association.targetRef });
        }
      }
      ...
      for (const association of dataOutputAssociations || []) {
        ...
        processGraph.addEdge({ source, target, id: association.id });
      }
    ...
    // todo: костыль добавляем базовый graph возможно стоит подменить
    grid.baseGraph = processGraph;
