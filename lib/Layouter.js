...
export class Layouter {
  ...
  createParticipantDi(participant, origin) {
    ...
    const nestedLanes = this.getLanesNestedSet(participant.processRef);
    const maxLevel = [ ...nestedLanes ].reduce((prev, [ , nestPos ]) => {
      return prev < nestPos.level ? nestPos.level : prev;
    },0);
    const labelWidth = 30;

    if (nestedLanes.size > 0) {
      height = rowCount * DEFAULT_CELL_HEIGHT;

      // 1 - лейбл пула
      // maxLevel + 1 level count
      width = width + (maxLevel + 2) * labelWidth;
    } else {
    ...
    nestedLanes.forEach((pos, lane) => {

      const laneX = poolOrigin.x + (pos.level + 1) * labelWidth;
      const leaves = [ ...nestedLanes ].filter(([ , itemPos ]) => {
      ...
      let laneWidth = (colCount > 0 ? colCount + 1 : 2) * DEFAULT_CELL_WIDTH + (levelDif + 1) * labelWidth;
  ...
  generateDi(process , shift, procDi) {
    ...
    // 1 - лейбл пула
    // maxLevel + 1 level count
    const elXShift = (maxLevel + 2) * labelWidth;
