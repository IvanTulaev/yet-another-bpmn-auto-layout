...
export class Layouter {
  ...
  drawStraight(message) {
    ...
    if (!existNodes.includes(sourceRef) || !existNodes.includes(targetRef)) return;
  ...
  createGridLayout(process) {
    ...
    for (const node of processGraph.nodes) {
      ...
      // DataStoreReference и DataObjectReference связываем с первым лейном процесса
      if (node.$type === 'bpmn:DataStoreReference' || node.$type === 'bpmn:DataObjectReference') {
        if (!node.laneRef && process.lanesNestedSet?.nestedSet.size > 0) {
          const firstLane = process.lanesNestedSet.getLeaves().sort(([ , a ], [ , b ]) => a.left - b.left)[0][0];
          node.laneRef = firstLane;
        }
      }
