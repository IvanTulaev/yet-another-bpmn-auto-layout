...
export class GridWithEdges extends Grid {
  ...
  getLanes() {
    return [ ...this.elements ].filter(element => element.$type === 'bpmn:Lane' || element.$type === 'bpmn:Process');
  }
  ...
  add(element, position) {
    ...
    if (!element.laneRef) throw new Error (`Element ${element.id} is not associated with any leaf in lane tree of the process ${element.$parent.id}.`);
  ...
  hasAnyCross(edges) {
    ...
    return [ ...executedEdges ].some(edge => {
      const hasHorizontalCross = this.getCrossedElementsFor(edge).filter(item => item.$type !== 'bpmn:Lane' && item.$type !== 'bpmn:Process').length > 0;
      if (hasHorizontalCross) return true;

      const verticalCross = this.getCrossedElementsFor(edge, true).filter(item => item.$type !== 'bpmn:Lane' && item.$type !== 'bpmn:Process').length > 0;
  ...
  shakeIt(byVertical) {
    ...
    const sortedElements = [ ...this.elements ].filter(item => item.$type !== 'bpmn:Lane' && item.$type !== 'bpmn:Process').sort(byVertical ? sortElementsTopRightBottomLeft(this) : sortColsLeftRightRowsBottomTop (this)).reverse();
      ...
      for (let index = byVertical ? baseRow - 1 : baseCol - 1 ; index >= 0; index--) {
        ...
        const newPositionsAreFine = [ ...chain ].every(element => {
          ...
          return ![ ...(this.get(checkedRow, checkedCol) || []) ]
            .filter(item => item.$type !== 'bpmn:Lane' && item.$type !== 'bpmn:Process')
            .length > 0;
        });
  ...
  getChain(element, byVertical, oldChain) {
    ...
    const elInPos = [ ...this.get(elementPosition[0], elementPosition[1]) ].filter(item => item.$type !== 'bpmn:Lane' && item.$type !== 'bpmn:Process');
  ...
  _pathForSouthEastToNorthWest(edge) {
    ...
    // и если есть входящее в source c направлениями NW_SE || W_E
    const source = this.getEdgeSource(edge);
    const hasBackward = this.getExistingIncomingEdgesFor(source).some(incoming => {
      const direction = this.getEdgeDirection(incoming);
      return direction === 'NW_SE' || direction === 'W_E';
    });
    if ((!(this.getEdgeSource(edge).$type === 'bpmn:BoundaryEvent' && edge.id)) && !hasBackward) {
  ...
  shrink(byVertical) {
    ...
    if (byVertical && this.getLanes().length > 0) {
      ...
      for (const element of [ ...this.elements ].filter(item => item.$type !== 'bpmn:Lane' && item.$type !== 'bpmn:Process')) {
