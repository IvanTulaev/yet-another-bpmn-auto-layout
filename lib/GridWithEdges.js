...
export class GridWithEdges extends Grid {
  ...
  isIntersect(edge, position, byVertical) {
    ...
    if (direction === 'SE_NW') {
      ...
      if (!byVertical && col >= targetCol && col < sourceCol && row === sourceRow && !this.getExistingIncomingEdgesFor(this.getEdgeSource(edge)).some(edge => this.getEdgeDirection(edge) === 'NW_SE' || this.getEdgeDirection(edge) === 'W_E')) return true;
      return false;
    }
  }
